import {
  buildInventoryRiskMap,
  classifyInventoryIndicator,
...
const DICTAMEN_DESCRIPTION: Record<string, string> = {
  favorable: 'Los niveles de inventario no muestran riesgos relevantes para la junta.',
  favorable_con_observaciones: 'Inventario en general sano, con puntos específicos a vigilar.',
  requiere_atencion: 'Riesgo de quiebre, sobrestock u obsolescencia en zona de alerta — requiere seguimiento cercano.',
  critico: 'Riesgo real para la continuidad operativa o el capital de trabajo — se requiere acción inmediata.',
};
...
function findSeccion(narrative: InventoryNarrative | null | undefined, titulo: string): string | undefined {
  return narrative?.secciones?.find((s) => s.titulo === titulo)?.analisis;
}

export function InventoryAnalysisDetail({
  id,
  title,
  companyName,
  periodStart,
  ...
  const r = (results && typeof results === 'object' ? results : {}) as Partial<InventoryAnalyticsResult>;
  const items = r.items ?? [];
  ...
  const kpiCards = hasData
    ? [
        { label: 'Valor Total Inventario', value: formatInventoryValue(resumen!.valorTotalInventario, 'currency'), status: 'neutral' as const },
        resumen!.rotacionAnualizada !== null
          ? { label: 'Rotación Anualizada', value: `${resumen!.rotacionAnualizada.toFixed(1)}x`, status: classifyInventoryIndicator('rotacion_anualizada', resumen!.rotacionAnualizada) }
          : null,
        resumen!.coberturaDiasPromedio !== null
          ? { label: 'Cobertura Promedio', value: formatInventoryValue(resumen!.coberturaDiasPromedio, 'days'), status: classifyInventoryIndicator('cobertura_dias_promedio', resumen!.coberturaDiasPromedio) }
          : null,
        {
          label: '% Valor en Riesgo de Quiebre',
          value: formatInventoryValue(resumen!.pctValorRiesgoQuiebre, 'percent'),
          status: classifyInventoryIndicator('pct_valor_riesgo_quiebre', resumen!.pctValorRiesgoQuiebre),
        },
    ...
    <div className="space-y-6">
      ...
      {!narrative && (
        <section className="rounded-xl border border-dashed border-slate-300 bg-white p-8 text-center shadow-sm">
          <p className="text-sm text-slate-500">
            ...
            indicador de inventario a partir del archivo cargado, o a que la generación con IA falló.
          </p>
        </section>
      )}
      ...
      {hasData ? (
        <>
          ...
          <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
            <h2 className="mb-3 text-base font-semibold text-slate-900">Valorización y Niveles de Stock</h2>
            {findSeccion(narrative, 'Valorización y Niveles de Stock') && (
              <p className="mb-4 text-sm leading-relaxed text-slate-600">{findSeccion(narrative, 'Valorización y Niveles de Stock')}</p>
            )}
            <InventoryTopBarChart items={items} />
                ...
                <table className="w-full text-left text-sm">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {valorizacionPorCategoria.map((c) => (
                      <tr key={c.categoria}>
                        <td className="py-2.5 pr-4 font-medium text-slate-800">{c.categoria}</td>
                        <td className="py-2.5 pr-4 text-right text-slate-600">{formatInventoryValue(c.valor, 'currency')}</td>
                        <td className="py-2.5 pr-4 text-right text-slate-500">{c.pctTotal.toFixed(1)}%</td>
                        <td className="py-2.5 text-right text-slate-500">{c.numSkus}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            {comparativo && (
              <div className="mt-6 grid grid-cols-2 gap-4 border-t border-slate-100 pt-6 sm:grid-cols-4">
                {Object.entries(comparativo.indicadores).map(([key, entry]) => {
                  ...
                  const label = key === 'valor_total_inventario' ? 'Valor Total' : key === 'rotacion_anualizada' ? 'Rotación Anual.' : key === 'cobertura_dias_promedio' ? 'Cobertura' : '% Riesgo Quiebre';
                  return (
                    <div key={key}>
                      <p className="text-xs uppercase tracking-wide text-slate-400">{label}</p>
                      <p className="mt-1 text-lg font-semibold text-slate-900">{formatInventoryValue(entry.valor_actual, format as any)}</p>
                      <p className="text-xs text-slate-500">
                        vs {formatInventoryValue(entry.valor_anterior, format as any)}
          ...
          <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
            <h2 className="mb-3 text-base font-semibold text-slate-900">Rotación y Cobertura de Inventario</h2>
            {findSeccion(narrative, 'Rotación y Cobertura de Inventario') && (
              <p className="mb-4 text-sm leading-relaxed text-slate-600">{findSeccion(narrative, 'Rotación y Cobertura de Inventario')}</p>
            )}
              ...
              <table className="w-full text-left text-sm">
                <thead>
                  <tr className="border-b border-slate-200 text-xs uppercase tracking-wide text-slate-400">
                    ...
                    <th className="py-2 pr-4 text-right font-medium">Rotación</th>
                ...
                <tbody className="divide-y divide-slate-100">
                  {items.slice(0, 15).map((it) => (
                    <tr key={it.sku}>
                      <td className="py-2.5 pr-4 font-medium text-slate-800">{it.sku}</td>
                      <td className="py-2.5 pr-4 text-slate-500">{it.categoria ?? '—'}</td>
                      <td className="py-2.5 pr-4 text-right text-slate-600">{it.stock}</td>
                      <td className="py-2.5 pr-4 text-right text-slate-500">{it.rotacionPeriodo !== null ? `${it.rotacionPeriodo.toFixed(2)}x` : '—'}</td>
                      <td className="py-2.5 pr-4 text-right text-slate-500">{it.coberturaDias !== null ? `${it.coberturaDias.toFixed(0)} d` : '—'}</td>
                      <td className="py-2.5">
                        <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${ABC_BADGE[it.claseAbc]}`}>{it.claseAbc}</span>
                      </td>
                    </tr>
                  ))}
                </tbody>
          ...
          {/* ── 3. Clasificación ABC ── */}
          <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
            <h2 className="mb-3 text-base font-semibold text-slate-900">Clasificación ABC</h2>
            {findSeccion(narrative, 'Clasificación ABC') && (
              <p className="mb-4 text-sm leading-relaxed text-slate-600">{findSeccion(narrative, 'Clasificación ABC')}</p>
            )}
            <InventoryAbcChart items={items} />

            {abcResumenPorClase.length > 0 && (
              <div className="mt-6 grid gap-4 border-t border-slate-100 pt-6 sm:grid-cols-2">
                <ComposicionPieChart title="Valor por Clase ABC" data={abcPieData} />
                <table className="w-full text-left text-sm">
                  <thead>
                    <tr className="border-b border-slate-200 text-xs uppercase tracking-wide text-slate-400">
                      <th className="py-2 pr-4 font-medium">Clase</th>
                      <th className="py-2 pr-4 text-right font-medium">SKUs</th>
                      <th className="py-2 pr-4 text-right font-medium">% Catálogo</th>
                    ...
                    {abcResumenPorClase.map((c) => (
                      <tr key={c.clase}>
                        <td className="py-2.5 pr-4">
                          <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${ABC_BADGE[c.clase]}`}>{c.clase}</span>
                        </td>
                        <td className="py-2.5 pr-4 text-right text-slate-600">{c.numSkus}</td>
                        <td className="py-2.5 pr-4 text-right text-slate-500">{c.pctSkus.toFixed(1)}%</td>
                        <td className="py-2.5 text-right font-medium text-slate-700">{c.pctValor.toFixed(1)}%</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
            <h2 className="mb-3 text-base font-semibold text-slate-900">Riesgo de Quiebre y Obsolescencia</h2>
            {findSeccion(narrative, 'Riesgo de Quiebre y Obsolescencia') && (
              <p className="mb-4 text-sm leading-relaxed text-slate-600">{findSeccion(narrative, 'Riesgo de Quiebre y Obsolescencia')}</p>
            )}
            ...
            {riesgoQuiebre.length > 0 ? (
                ...
                <table className="w-full text-left text-sm">
                  <thead>
                    <tr className="border-b border-slate-200 text-xs uppercase tracking-wide text-slate-400">
                      ...
                      <th className="py-2 pr-4 text-right font-medium">Stock</th>
                      <th className="py-2 pr-4 text-right font-medium">Cobertura</th>
                      <th className="py-2 pr-4 text-right font-medium">Lead Time</th>
                      <th className="py-2 font-medium">Diagnóstico</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100">
                    {riesgoQuiebre.map((it) => (
                      <tr key={it.sku} className="bg-red-50/40">
                        <td className="py-2.5 pr-4 font-medium text-slate-800">{it.sku}</td>
                        <td className="py-2.5 pr-4 text-slate-500">{it.categoria ?? '—'}</td>
                        <td className="py-2.5 pr-4 text-right text-slate-600">{it.stock}</td>
                        <td className="py-2.5 pr-4 text-right text-slate-500">{it.coberturaDias !== null ? `${it.coberturaDias.toFixed(0)} d` : '—'}</td>
                        <td className="py-2.5 pr-4 text-right text-slate-500">{it.leadTimeDias !== null ? `${it.leadTimeDias} d` : '—'}</td>
                        <td className="py-2.5 font-medium text-red-700">{it.diagnostico}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <p className="text-sm text-slate-400">No se identificaron SKUs en riesgo de quiebre.</p>
            )}

            {obsolescencia.length > 0 && (
              <div className="mt-6 border-t border-slate-100 pt-6">
                <h3 className="mb-3 text-sm font-semibold text-slate-900">Inventario Obsoleto / Dead Stock</h3>
                <div className="overflow-x-auto">
                  <table className="w-full text-left text-sm">
                    <thead>
                      <tr className="border-b border-slate-200 text-xs uppercase tracking-wide text-slate-400">
                        <th className="py-2 pr-4 font-medium">SKU</th>
                        <th className="py-2 pr-4 font-medium">Categoría</th>
                        <th className="py-2 pr-4 text-right font-medium">Valor Inmovilizado</th>
                        <th className="py-2 font-medium">Diagnóstico</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100">
                      {obsolescencia.slice(0, 15).map((it) => (
                        <tr key={it.sku} className="bg-amber-50/40">
                          <td className="py-2.5 pr-4 font-medium text-slate-800">{it.sku}</td>
                          <td className="py-2.5 pr-4 text-slate-500">{it.categoria ?? '—'}</td>
                          <td className="py-2.5 pr-4 text-right text-slate-600">{formatInventoryValue(it.valorInventario, 'currency')}</td>
                          <td className="py-2.5 font-medium text-amber-700">{it.diagnostico}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            {sobrestock.length > 0 && (
              <div className="mt-6 border-t border-slate-100 pt-6">
                <h3 className="mb-3 text-sm font-semibold text-slate-900">Sobrestock</h3>
                <div className="overflow-x-auto">
                  <table className="w-full text-left text-sm">
                    <thead>
                      <tr className="border-b border-slate-200 text-xs uppercase tracking-wide text-slate-400">
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {sobrestock.slice(0, 15).map((it) => (
                        <tr key={it.sku}>
                          <td className="py-2.5 pr-4 font-medium text-slate-800">{it.sku}</td>
                          <td className="py-2.5 pr-4 text-slate-500">{it.categoria ?? '—'}</td>
                          <td className="py-2.5 pr-4 text-right text-slate-600">{formatInventoryValue(it.valorInventario, 'currency')}</td>
                          <td className="py-2.5 text-slate-600">{it.diagnostico}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            {(controlEstado || proveedores) && (
              <div className="mt-6 grid gap-6 border-t border-slate-100 pt-6 sm:grid-cols-2">
                {controlEstado && (
                  <div>
                    <h3 className="mb-3 text-sm font-semibold text-slate-900">Control de Estado</h3>
                    <table className="w-full text-left text-sm">
                      <tbody className="divide-y divide-slate-100">
                        {controlEstado.map((e) => (
                          <tr key={e.estado}>
                            <td className="py-2 pr-4 capitalize text-slate-700">{e.estado}</td>
                            <td className="py-2 pr-4 text-right text-slate-500">{e.numSkus} SKU(s)</td>
                            <td className="py-2 text-right font-medium text-slate-700">{formatInventoryValue(e.valor, 'currency')} ({e.pctValor.toFixed(1)}%)</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                ...
                {proveedores && (
                  <div>
                    <h3 className="mb-3 text-sm font-semibold text-slate-900">Concentración por Proveedor</h3>
                    <table className="w-full text-left text-sm">
                      <tbody className="divide-y divide-slate-100">
                        {proveedores.slice(0, 8).map((p) => (
                          <tr key={p.proveedor}>
                            <td className="py-2 pr-4 text-slate-700">{p.proveedor}</td>
                            <td className="py-2 pr-4 text-right text-slate-500">{p.numSkus} SKU(s)</td>
                            <td className="py-2 text-right font-medium text-slate-700">{formatInventoryValue(p.valor, 'currency')} ({p.pctTotal.toFixed(1)}%)</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
        ...
        <div className="rounded-xl border border-dashed border-slate-300 bg-white p-10 text-center shadow-sm">
          <p className="text-sm text-slate-500">Este análisis no tiene inventario calculado a partir del archivo cargado.</p>
        </div>
      )}
